...
export class AuthRoute implements Routes {
  ...
  private initializeRoutes() {
    this.router.post('/auth/signup', ValidationMiddleware(RegisterUserDto), this.auth.signUp);
    this.router.post('/auth/login', ValidationMiddleware(LoginUserDto), this.auth.logIn);
    this.router.post('/auth/logout', AuthMiddleware(), ValidationMiddleware(LogoutUserDto), this.auth.logOut);
